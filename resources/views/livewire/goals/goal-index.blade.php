...
@endphp
<div>
    <x-common.page-breadcrumb title="Metas" />
    ...
    <x-ui.card :padding="false" class="pt-4">
        ...
        <div class="max-w-full px-5 overflow-x-auto">
            <x-ui.data-table hoverable>
                ...
                @forelse ($goals as $goal)
                    <tr wire:key="{{ $goal->id }}"
                        @class(['transition-colors duration-200', 'relative' => is_array($selected) && in_array($goal->id, $selected)])
                        @style(['background-color: rgba(251, 101, 20, 0.15)' => is_array($selected) && in_array($goal->id, $selected)])
                    >
                        <td class="px-4 py-4 whitespace-nowrap" @if(is_array($selected) && in_array($goal->id, $selected)) style="border-left: 3px solid #fb6514;" @endif>
                            <x-ui.checkbox wire:model.live="selected" value="{{ $goal->id }}" />
                        </td>
                        ...
                        <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                            <div class="flex justify-center gap-2">
                                <button wire:click="edit({{ $goal->id }})" class="{{ $btnIcon }} text-green-600" style="background-color: rgba(22, 163, 74, 0.1);">
                                    <svg
                                ...
                                <button wire:click="confirmDelete({{ $goal->id }})" class="{{ $btnIcon }}" style="{{ $btnDangerStyle }}">
                    ...
                    <tr>
                        <td colspan="8" class="p-0">
                            <x-ui.empty-state title="Nenhuma meta encontrada"
                                description="Tente ajustar sua busca ou crie uma nova meta." />
                        </td>
                    </tr>
                @endforelse
            </x-ui.data-table>
        ...
        <div class="flex items-center flex-col sm:flex-row justify-between px-5 py-4" style="border-top: 1px solid var(--border-color);">
            <x-ui.pagination :paginator="$goals" />
        </div>
    </x-ui.card>
    ...
    {{-- Create Modal --}}
    @if($showCreateModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm" style="background-color: rgba(0, 0, 0, 0.5);" wire:click="closeCreateModal" x-transition>
            <div class="w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl p-6" style="background-color: var(--bg-surface); border: 1px solid var(--border-color);" wire:click.stop>
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-xl font-semibold" style="color: var(--text-primary);">Criar Nova Meta</h3>
                ...
                <form wire:submit="save" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium mb-2" style="color: var(--text-secondary);">Métrica *</label>
                            <x-form.multiple-select
                                wire:model="metric"
                                :multiple="false"
                                :options="[
                                    ...
                                    ['value' => 'revenue', 'label' => 'Renda']
                                ]"
                            />
                            @error('metric') <span class="text-red-400 text-sm">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-2" style="color: var(--text-secondary);">Período *</label>
                            <x-form.multiple-select
                                wire:model="period"
                                :multiple="false"
                                :options="[
                    ...
                    <x-ui.input label="Valor Alvo *" wire:model="target_value" type="number" step="0.01" :error="$errors->first('target_value')" />
    ...
    {{-- Edit Modal --}}
    @if($showEditModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm" style="background-color: rgba(0, 0, 0, 0.5);" wire:click="closeEditModal" x-transition>
            <div class="w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl p-6" style="background-color: var(--bg-surface); border: 1px solid var(--border-color);" wire:click.stop>
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-xl font-semibold" style="color: var(--text-primary);">Editar Meta</h3>
    ...
    @if($showDeleteModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center px-4 backdrop-blur-sm" style="background-color: rgba(0, 0, 0, 0.5);" wire:click="cancelDelete">
            <div class="w-full max-w-md rounded-2xl shadow-2xl p-6" style="background-color: var(--bg-surface); border: 1px solid var(--border-color);" wire:click.stop>
