        ...
        <div class="max-w-full px-5 overflow-x-auto">
            <x-ui.data-table hoverable>
                ...
                @forelse($categories as $category)
                    <tr wire:key="{{ $category->id }}" @class(['transition-colors duration-200', 'relative' => is_array($selected) && in_array($category->id, $selected)])
                        @style(['background-color: rgba(251, 101, 20, 0.15)' => is_array($selected) && in_array($category->id, $selected)])>
                        <td class="px-4 py-4 whitespace-nowrap" @if(is_array($selected) && in_array($category->id, $selected))
                        style="border-left: 3px solid #fb6514;" @endif>
                            <x-ui.checkbox wire:model.live="selected" value="{{ $category->id }}" />
                        </td>
                        ...
                        <td class="px-4 py-4 whitespace-nowrap text-center text-sm" style="color: var(--text-secondary);">
                            {{ $category->challenges_count }}
                        </td>

                        <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                            <div class="flex justify-center gap-2">
                                <button wire:click="edit({{ $category->id }})" class="{{ $btnIcon }} text-green-600" style="background-color: rgba(22, 163, 74, 0.1);">
                                ...
                                <button wire:click="confirmDelete({{ $category->id }})" class="{{ $btnIcon }}" style="{{ $btnDangerStyle }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                        fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                ...
                @empty
                    <tr>
                        <td colspan="4" class="p-0">
                            <x-ui.empty-state title="Nenhuma categoria encontrada"
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm" style="background-color: rgba(0, 0, 0, 0.5);"
            ...
            <div class="w-full max-w-md mx-4 rounded-2xl shadow-2xl p-6" style="background-color: var(--bg-surface); border: 1px solid var(--border-color);" wire:click.stop>
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button wire:click="$set('showModal', false)" class="p-1 rounded-lg transition-colors" style="color: var(--text-secondary);">
                ...
                <form wire:submit.prevent="save" class="space-y-4">
                    <x-ui.input label="Nome da categoria" wire:model="name" type="text" :error="$errors->first('name')" />

                    <x-ui.field label="Cor" :error="$errors->first('color')">
                        <div class="mt-2 grid grid-cols-5 sm:grid-cols-6 gap-2">
                            @foreach($availableColors as $colorCode => $colorName)
                                <label class="relative flex items-center justify-center cursor-pointer">
                                    <input type="radio" wire:model.live="color" value="{{ $colorCode }}" class="sr-only">
                    ...
                    <div class="flex justify-end gap-3 pt-4">
                        <button type="button" wire:click="$set('showModal', false)" class="{{ $btnSecondary }}" style="{{ $btnSecondaryStyle }}">
                            Cancelar
                        </button>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm" style="background-color: rgba(0, 0, 0, 0.5);"
            ...
            <div class="w-full max-w-md mx-4 rounded-2xl shadow-2xl p-6" style="background-color: var(--bg-surface); border: 1px solid var(--border-color);" wire:click.stop>
                <div class="flex justify-between items-start mb-4">
                    ...
                    <button wire:click="$set('confirmingDeletion', false)" class="p-1 rounded-lg transition-colors" style="color: var(--text-secondary);">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </button>
                ...
                <p class="text-sm mb-6 ml-16" style="color: var(--text-secondary);">
                    Tem certeza que deseja excluir a categoria "<strong>{{ $categoryToDelete?->name }}</strong>"?
                    Esta ação não pode ser desfeita.
                </p>

                <div class="flex justify-end gap-3">
                    <button type="button" wire:click="$set('confirmingDeletion', false)" class="{{ $btnSecondary }}" style="{{ $btnSecondaryStyle }}">
                        Cancelar
                    </button>
