...
<div class="rounded-2xl border border-zinc-200 bg-white pt-4 dark:border-zinc-800 dark:bg-white/[0.03]">
    ...
    <div class="flex flex-col gap-2 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        ['value' => -1, 'label' => 'Todos']
            ...
            <div class="relative z-20 w-40">
                <x-form.multiple-select wire:model.live="sportType" :multiple="false" placeholder="Todos os esportes"
                    :options="$sportTypes->map(fn($type) => ['value' => $type, 'label' => ucfirst($type)])->toArray()" />
            </div>
        </div>
        ...
        <table class="min-w-full">
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                @forelse($segments as $segment)
                    <tr wire:key="{{ $segment->id }}" @class(['hover:bg-zinc-50 dark:hover:bg-zinc-700/50 transition-colors duration-200', 'relative' => is_array($selected) && in_array($segment->id, $selected)])
                        @style(['background-color: rgba(251, 101, 20, 0.15)' => is_array($selected) && in_array($segment->id, $selected)])>
                        <td class="px-4 py-4 whitespace-nowrap" @if(is_array($selected) && in_array($segment->id, $selected))
                        style="border-left: 3px solid #fb6514;" @endif>
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" wire:model.live="selected" value="{{ $segment->id }}" class="sr-only peer" />
                        ...
                        <td class="px-4 py-4 text-sm font-medium text-right whitespace-nowrap">
                            <div class="flex justify-center gap-2">
                                <button wire:click="viewLeaderboard({{ $segment->id }})"
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                        ...
                                        stroke-linejoin="round" class="lucide lucide-trophy w-5 h-5">
                                ...
                                <button wire:click="confirmDelete({{ $segment->id }})"
    ...
    <div
        ...
        {{ $segments->links('components.pagination.custom') }}
    </div>
</div>
    ...
    <div class="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50"
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto"
            ...
            <div class="flex justify-between items-center mb-4">
                <div>
                    <h3 class="text-xl font-semibold text-zinc-900 dark:text-zinc-100">Leaderboard — {{ $selectedSegment->name }}</h3>
                    <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ ucfirst($selectedSegment->sport_type) }} · {{ $selectedSegment->efforts_count }} tentativas registradas</p>
                </div>
                <button wire:click="closeLeaderboardModal" class="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200">
            ...
            @if($leaderboard->isEmpty())
                <p class="text-zinc-500 dark:text-zinc-400 text-sm py-6 text-center">Nenhuma tentativa registrada para este segmento ainda.</p>
            @else
                <div class="max-w-full overflow-x-auto">
                    <table class="min-w-full">
                        <thead class="border-t border-zinc-100 border-y bg-zinc-50 dark:bg-zinc-900">
                            <tr class="border-zinc-200 border-y dark:border-zinc-700">
                                <th class="px-3 py-2 font-normal text-zinc-500 text-start text-theme-sm dark:text-zinc-400">#</th>
                                <th class="px-3 py-2 font-normal text-zinc-500 text-start text-theme-sm dark:text-zinc-400">Atleta</th>
                                <th class="px-3 py-2 font-normal text-zinc-500 text-start text-theme-sm dark:text-zinc-400">Tempo</th>
                                <th class="px-3 py-2 font-normal text-zinc-500 text-start text-theme-sm dark:text-zinc-400">Data</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
            ...
            <div class="flex items-center gap-4 mb-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-sm text-zinc-600 dark:text-zinc-400 mt-1">
                        Tem certeza que deseja excluir este segmento? Todas as tentativas registradas para ele também serão removidas. Esta ação não pode ser desfeita.
                    </p>
                </div>
            </div>
