...
<div class="rounded-2xl border border-zinc-200 bg-white pt-4 dark:border-zinc-800 dark:bg-white/[0.03]">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            <form>
                <div class="relative">
                    ...
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Pesquisar no texto do comentário..."
        ...
        <table class="min-w-full">
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                @forelse($comments as $comment)
                    <tr wire:key="{{ $comment->id }}" @class(['hover:bg-zinc-50 dark:hover:bg-zinc-700/50 transition-colors duration-200', 'relative' => is_array($selected) && in_array($comment->id, $selected)])
                        @style(['background-color: rgba(251, 101, 20, 0.15)' => is_array($selected) && in_array($comment->id, $selected)])>
                        <td class="px-4 py-4 whitespace-nowrap" @if(is_array($selected) && in_array($comment->id, $selected))
                        style="border-left: 3px solid #fb6514;" @endif>
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" wire:model.live="selected" value="{{ $comment->id }}" class="sr-only peer" />
                        ...
                        <td class="px-4 py-4">
                            ...
                            @if ($comment->parent_id)
                                <div class="text-xs text-zinc-400 dark:text-zinc-500 mt-1 italic">
                        ...
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="text-sm text-zinc-500 dark:text-zinc-400">
                                ...
                                @else
                                    <span class="text-xs text-zinc-400">{{ class_basename($comment->commentable_type) }}</span>
                                @endif
                            </div>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
    ...
    <div
        ...
        {{ $comments->links('components.pagination.custom') }}
    </div>
</div>
